...
//		this.drawingMode = true;
	...
	public void mousePressed(MouseEvent e) {
		lastClickedTileX = e.getX() / Constants.tileWidth;
		lastClickedTileY = e.getY() / Constants.tileHeight;
	...
	private void updateTile(int xCor, int yCor) {
		xCor = Math.max(0, Math.min(xCor, Constants.gridWidth -1));
		yCor = Math.max(0, Math.min(yCor, Constants.gridHeight -1));
	...
	public void mouseDragged(MouseEvent e) {
		if (ifRightMouseButtonPressed(e)) {
			int newTileX = e.getX() / Constants.tileWidth;
			int newTileY = e.getY() / Constants.tileHeight;
