...
/* 
    Perform insertion sort to the watchtowers array on the diameter of the cell in ascending order
    Reference from the pseudocode in the specs
*/
void insertionSortWt(struct DCEL *dcel, struct watchtowerStruct **wts, int wtCount);

void insertionSortWt(struct DCEL *dcel, struct watchtowerStruct **wts, int wtCount) {
    int i, j;
    double currentDiameter;
    struct watchtowerStruct *currentWt;
    for(i = 1; i < wtCount; i++) {
        currentWt = wts[i];
        currentDiameter = getDiameter(dcel, currentWt->face);
        j = i - 1;

        while(j >= 0 && getDiameter(dcel, wts[j]->face) > currentDiameter) {
            wts[j+1] = wts[j];
            j--;
        }
        wts[j+1] = currentWt;
...
void outputResultDiameterSorted(char *outputfileName, struct watchtowerStruct **wts, int wtCount, 
    ...
    */
    ...
    insertionSortWt(dcel, wts, wtCount);
